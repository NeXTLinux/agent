		...
		if !found {
			...
			if isSYN {
				if netMonitor.Status == "Dropped" {
					...
					if ipv4Address != NextLinuxSinkHoleIPAddress { // Sinkhole IP address will be covered by DNS block
						go WriteAnnotation(fmt.Sprintf("NextLinux Harden Runner: Traffic to IP Address %s was blocked", ipv4Address))
					}
				}
			}
		}
