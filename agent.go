...
const (
	NextLinuxLogCorrelationPrefix = "Step Security Job Correlation ID:"
	NextLinuxAnnotationPrefix     = "NextLinux Harden Runner:"
...
func Run(ctx context.Context, configFilePath string, hostDNSServer DNSServer,
	...
	WriteLog(fmt.Sprintf("%s %s", NextLinuxLogCorrelationPrefix, config.CorrelationId))
	WriteLog("\n")
	...
	if config.Private {
		...
		if !isActive {
			...
			WriteAnnotation("NextLinux Harden Runner is disabled. A subscription is required for private repositories. Please start a free trial at https://www.next-linux.systems")
		}
	}
	...
	if config.EgressPolicy == EgressPolicyBlock {
		for domainName, endpoints := range allowedEndpoints {
			...
			if err != nil {
				...
				WriteAnnotation(fmt.Sprintf("%s Reverting agent since allowed endpoint %s could not be resolved", NextLinuxAnnotationPrefix, strings.Trim(domainName, ".")))
	...
	if config.DisableSudo {
		...
		if err != nil {
			WriteAnnotation(fmt.Sprintf("%s Unable to disable sudo %v", NextLinuxAnnotationPrefix, err))
		} else {
...
func addImplicitEndpoints(endpoints map[string][]Endpoint, disableTelemetry bool) (map[string][]Endpoint, map[string][]Endpoint) {
	...
	nextlinux := Endpoint{domainName: "agent.api.next-linux.systems", port: 443} // Should be implicit based on user feedback
	if !disableTelemetry {
		// allowing only if disable_telemetry is set to false
		normalEndpoints[nextlinux.domainName] = append(normalEndpoints[nextlinux.domainName], nextlinux)
	}
