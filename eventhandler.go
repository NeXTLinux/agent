...
func (eventHandler *EventHandler) handleFileEvent(event *Event) {
	...
	if strings.Contains(event.FileName, "daemon.json") {
		WriteAnnotation(fmt.Sprintf("%s Overwrite detected for %s", NextLinuxAnnotationPrefix, dockerDaemonConfigPath))
	}

	if strings.Contains(event.FileName, "resolved.conf") {
		WriteAnnotation(fmt.Sprintf("%s Overwrite detected for %s", NextLinuxAnnotationPrefix, resolvedConfigPath))
	}
			...
			if isFromDifferentProcess {
				...
				if !found || counter < 3 {
					...
					if err == nil {
						...
						// WriteAnnotation(fmt.Sprintf("NextLinux Harden Runner: Source code overwritten file: %s syscall: %s by exe: %s", event.FileName, event.Syscall, event.Exe))
					}
