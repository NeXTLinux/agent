...
const NextLinuxSinkHoleIPAddress = "54.185.253.63"
		...
		if !proxy.isAllowedDomain(domain) {
			...
			if !matchesAnyWildcard {
				...
				if !strings.Contains(domain, "api.snapcraft.io") && !strings.Contains(domain, "docker.io") {
					go WriteAnnotation(fmt.Sprintf("NextLinux Harden Runner: DNS resolution for domain %s was blocked. This domain is not in the list of allowed-endpoints.", domain))
				}

				// return an ip address, so calling process calls the ip address
				// the call will be blocked by the firewall
				proxy.Cache.Set(domain, &Answer{Name: domain, TTL: math.MaxInt32, Data: NextLinuxSinkHoleIPAddress}, false)

				go proxy.ApiClient.sendDNSRecord(proxy.CorrelationId, proxy.Repo, domain, NextLinuxSinkHoleIPAddress)

				return NextLinuxSinkHoleIPAddress, nil
			}
		}
	}
